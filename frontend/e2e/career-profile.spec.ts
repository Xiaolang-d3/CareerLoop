...
test("empty-to-confirmed profile governance path", async ({ page }) => {
  ...
  await page.route("http://127.0.0.1:8000/**", async (route) => {
    ...
    if (path === "/agent/settings") return route.fulfill({ json: { display_name: "BossCopilot", persona_role: "求职顾问", response_style: "concise", custom_instructions: "", profile_memory_enabled: true, conversation_memory_enabled: true, knowledge_memory_enabled: true, summary_enabled: true, context_message_limit: 12, model_name: "gpt", model_base_url: "", api_key: "", api_key_configured: false } });
