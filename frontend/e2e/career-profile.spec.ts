...
test("saved profile and resume remain available in settings", async ({ page }) => {
  const profile: any = {
    profile: {
      id: 1,
      name: "端到端用户",
  ...
  await expect(page.getByText("简历已导入")).toBeVisible();
  await expect(page.getByRole("heading", { name: "简历预览" })).toBeVisible();
