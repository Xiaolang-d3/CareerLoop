...
本地优先的 AI 求职工作台。CareerLoop 把分散的简历、项目经历、岗位信息和面试复盘沉淀为可复用的职业证据，帮助用户持续管理机会、判断匹配、准备材料并推进面试。

当前版本为 **2.0.0 开发预览版**，尚未发布稳定的 GitHub Release。

## 核心能力

- **职业画像与证据**：维护脱敏简历、技能、目标与已确认事实，所有推导信息先进入待确认队列。
- **机会中心**：收集岗位和来源，按职业策略筛选、排序并跟踪推进状态。
- **岗位分析**：完成简历匹配、A–G 决策评估、岗位比较和风险核验。
- **简历工作室**：生成、编辑、预览和导出针对岗位的简历版本。
- **项目工作室**：从项目描述或公开 GitHub 仓库生成技术栈、架构、核心模块和表达 briefing。
- **面试准备与复盘**：围绕真实项目证据生成问题、知识点和差距，支持文字/语音练习与复盘记录。
- **受控求职 Agent**：按任务路由工具，限制计划外操作；联网结果要求引用，关键写入保留人工确认。
