...
- 候选人画像、简历和求职偏好
- 用户在当前对话主动粘贴 JD 或上传岗位截图
- JD 匹配分析、简历差距分析和简历证据检索
- 多对话、上下文记忆和 Agent 设置
- AG-UI 流式聊天和工具执行状态
- 本地附件或 MinIO 私有对象存储

这份列表是实现状态，不是产品范围。岗位平台、职位库、文件生成、沟通、投递和其他能力可以通过后续适配器或结构化工具接入。运行时必须按实际工具能力报告执行结果，涉及账号、发送和提交的工具需要定义明确的确认与审计机制。

## 阶段 0：能力开放与版本基线

### 目标

移除写死在接口、提示词和文档中的产品范围限制，同时形成可恢复、可审查的版本基线。

### 交付内容

- 统一 README、PRD、技术架构和实施路线
- 区分“当前尚未实现”和“产品禁止”，避免把实现状态固化为范围
- 将大规模工作区改动拆分为内聚提交
- 推送本地超前提交和新功能提交
- 记录当前数据库和附件兼容策略
...
- 上述流程可以在干净数据库中完整走通。
- 所有关键状态均可在重新启动后恢复。
- 每个分析结论能够追溯到当前 JD 或简历证据。
- 只有外部工具返回成功时，系统才会记录或声称已经完成发送或投递。
