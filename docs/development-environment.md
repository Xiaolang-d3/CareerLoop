...
The Fake Provider remains available for offline development. To enable an OpenAI-compatible model locally, create an ignored `backend/.env` from `backend/.env.example`, set `MODEL_PROVIDER=openai`, `MODEL_NAME`, `OPENAI_API_KEY`, and optionally `MODEL_BASE_URL`, then restart the backend. Never commit the local env file or print its secret values in logs.
