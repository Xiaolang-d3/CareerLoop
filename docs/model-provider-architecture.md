...
工具在 `backend/app/agent/bootstrap.py` 中注册，覆盖简历与 JD 分析、候选人画像与访谈、岗位评估与决策、机会发现与推进、材料生成与公开信息研究五个能力域。完整清单见 `docs/technical-architecture.md` 的“工具边界”一节，本文不重复维护。
...
当前工具包含只读、分析和本地写入三类；写入只落到本机 SQLite，例如岗位评估快照、面试复盘和待用户确认的画像补充。向招聘平台发送消息、投递或提交表单仍没有对应工具，也不在当前注册范围内。只有工具实际成功后，模型才能声称动作已经完成。
